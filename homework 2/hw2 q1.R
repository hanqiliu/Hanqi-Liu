...
# Firstly, we set up the medium model by hand, by adding the main 11 variables sequentially and the concerning interactions
...
# In order to judge the effect of different variables and interations, we set up the train sets and test sets to calculate the rmse of different models, and to figure out how much a variable and interation will affect the RMSE of the model.
...
# Relatively, we want to receive the smaller RMSE as we can. From the RMSE result above, we can see that by adding the interactions and the variables, the RMSE starts to decrease.
...
# Firstly, we set up the model matrix and scale it up, by considering the 11 main variables for the effects of the price.
